...
function Update-FileText {
    ...
    if ($content.IndexOf($OldStr) -lt 0) {
        Write-Warning "Pattern not found in $Path — manual update may be needed:`n  '$OldStr'"
        return
    }
...
# ── 2. docs/VERSIONING.md ─────────────────────────────────────────────────────
# Replace the alpha row state in the version history table
$baseVer = $stableVer  # e.g. "1.0.0"
